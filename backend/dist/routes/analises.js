...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const totalProcedimentosReaisAlunos = rondasAlunos.reduce((sum, r) => sum + (r.procedimentos_reais || 0), 0);
        const totalProcedimentosSimulados = rondasAlunos.reduce((sum, r) => sum + (r.procedimentos_simulados || 0), 0);
        ...
        const totalPacientesAvaliados = rondasAlunos.reduce((sum, r) => sum + (r.qtd_pacientes_avaliados || 0), 0);
        ...
        const mediaOciosidade = rondasAlunos.length > 0
            ? rondasAlunos.reduce((sum, r) => sum + (r.ociosidade || 0), 0) / rondasAlunos.length
            : 0;
        const totalProcedimentosPrevistosUti = rondasUtis.reduce((sum, r) => sum + (r.procedimentos_previstos || 0), 0);
        const totalProcedimentosReaisUti = rondasUtis.reduce((sum, r) => sum + (r.procedimentos_reais || 0), 0);
